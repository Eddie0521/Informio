...
export function SpreadsheetConflictDialog({
  open,
  fileName,
  hasUnsavedChanges,
  onReload,
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-[93] w-[min(480px,calc(100vw-40px))] -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-4 text-[var(--text-main)] shadow-[0_24px_64px_rgba(15,23,42,0.24),0_0_0_1px_rgba(15,23,42,0.08)] focus:outline-none">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <Dialog.Title className="text-[15px] font-extrabold">{t("spreadsheet.conflictTitle")}</Dialog.Title>
              <Dialog.Description className="mt-2 text-[13px] leading-6 text-[var(--text-muted)]">
                {t("spreadsheet.conflictDescription", { fileName })}
                {hasUnsavedChanges ? ` ${t("spreadsheet.conflictUnsavedWarning")}` : ""}
              </Dialog.Description>
            </div>
            <button
              type="button"
          ...
          <div className="mt-4 flex flex-wrap justify-end gap-2">
            ...
            <button
              ...
              onClick={onKeepLocal}
            >
              {t("spreadsheet.conflictKeepLocal")}
            </button>
            <button
              type="button"
