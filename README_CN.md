<p align="right">
  <a href="./README.md">English</a> | 简体中文 · <a href="https://informio.tech/cn">官网</a>
</p>
...
从源码打包或参与开发前，请先安装以下工具（下载 Release 的普通用户不需要）：

1. **Node.js 20 LTS 或更高版本**（自带 Corepack）
   - 官网安装：[nodejs.org](https://nodejs.org/)
   - macOS 也可用 Homebrew：`brew install node`
   - Windows 请下载 LTS 安装包，安装时勾选将 Node 加入 PATH
2. **启用 pnpm**（项目通过 Corepack 锁定版本，无需单独安装 pnpm）
   ```bash
   corepack enable
   ```
3. **Git**（用于克隆仓库）：[git-scm.com](https://git-scm.com/)
...
请先完成[前置条件](#前置条件)。Informio 使用 `package.json` 中声明的 `pnpm` 版本。macOS 和 Windows 都在仓库根目录运行下面的命令：
